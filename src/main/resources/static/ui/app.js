/* =========================================================
   SECURE CHAT — app.js
   All critical bugs fixed + new design-system rendering
========================================================= */
...
/* =========================================================
   ...
   BUG FIX: always encrypt for BOTH sender and receiver so
   each party can independently decrypt their own copy of
   the AES key. Without encryptedKeyForSender, the sender
   could not re-read their own sent messages.
========================================================= */
...
/* =========================================================
   DECRYPTION HELPERS
   BUG FIX: resolveKey picks encryptedKeyForSender when the
   logged-in user is the sender (they cannot decrypt with
   the receiver's wrapped key).
========================================================= */
...
/* =========================================================
   LOAD USERS
   BUG FIX: avatar gradient assigned by cycling through
   palette so each user gets a consistent color.
========================================================= */
...
/* =========================================================
   WEBSOCKET
   BUG FIX: _safeSubscribe prevents duplicate subscription
   stacking on reconnect (each reconnect previously added
   another listener, causing N-times message processing).
========================================================= */
...
/* =========================================================
   ...
   BUG FIX: uses sender/receiver email strings (not numeric IDs).
...
/* =========================================================
   SEND MESSAGE
   BUG FIX: always fetches receiver's current public key
   from server; never uses a stale cached version.
========================================================= */
...
/* =========================================================
   RENDER CHAT
   BUG FIX: alignment determined by comparing sender EMAIL
   string (not numeric senderId) to currentUserEmail.
   Shows a "Today" date chip divider.
========================================================= */
...
/* =========================================================
   FILE MESSAGE CARD
   BUG FIX: image preview uses a div for failure state —
   NEVER a broken <img alt=""> tag.
   BUG FIX: sender can also see preview (resolveKey picks
